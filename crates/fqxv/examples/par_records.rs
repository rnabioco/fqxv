...
//! optionally under a [`fqxv::StreamSelection`] (issue #272).
//!
...
//! reports wall time per repetition. The checksum makes the paths cross-check
//! each other: it is order-independent (a wrapping sum of per-record xxh3
//! digests), so serial and parallel runs of the same selection must agree
//! exactly.
//!
//! Usage: cargo run --release -p fqxv --example par_records -- \
//!   <archive.fqxv> <serial|par> [threads] [reps] [all|seq|nameseq|names|qual|none]
...
fn main() {
    ...
    let usage = "usage: par_records <archive.fqxv> <serial|par> [threads] [reps] \
                 [all|seq|nameseq|names|qual|none]";
    ...
    let sel_name = args.next().unwrap_or_else(|| "all".to_string());
    let selection = match sel_name.as_str() {
        "all" => fqxv::StreamSelection::ALL,
        "seq" => fqxv::StreamSelection::SEQUENCE_ONLY,
        "nameseq" => fqxv::StreamSelection::NAMES_AND_SEQUENCE,
        "names" => fqxv::StreamSelection::NAMES_ONLY,
        "qual" => fqxv::StreamSelection::QUALITY_ONLY,
        "none" => fqxv::StreamSelection::NONE,
        other => panic!("unknown selection {other:?}; {usage}"),
    };
    ...
    for rep in 0..reps {
        ...
        let acc = match mode.as_str() {
            "serial" => {
                ...
                for rec in fqxv::RecordReader::with_selection(file, threads, selection) {
            ...
            "par" => {
                let (acc, _stats) = fqxv::decompress_records_par_select(
                    file,
                    threads,
                    selection,
                    Acc::default,
        ...
        let dt = t0.elapsed();
        println!(
            "{mode}\tsel={sel_name}\tthreads={threads}\trep={rep}\t{:.3}s\trecords={}\tbases={}\tchecksum={:#018x}",
            dt.as_secs_f64(),
            acc.records,
            acc.bases,
            acc.checksum
