...
for DS in ecoli_ont ecoli_hifi; do
  ...
  echo "##### $DS  ($(stat -Lc %s "$IN") bytes)"
  for Q in org none; do
    rm -f "$O/$DS.$Q.cord"
    /usr/bin/time -f "  colord -q $Q: %e s" $CO compress-ont -t 64 -q $Q "$IN" "$O/$DS.$Q.cord" >/dev/null 2>>"$O/$DS.$Q.err"
    echo "  colord -q $Q  = $(stat -Lc %s "$O/$DS.$Q.cord") bytes"
  done
  ORG=$(stat -Lc %s "$O/$DS.org.cord"); NONE=$(stat -Lc %s "$O/$DS.none.cord")
  ...
  echo "  fqxv total = $(stat -Lc %s "$O/$DS.fqxv") bytes"
